 ...
 */
@Service
public class LongRuleService implements RuleService {
    ...
    public List<BetEntity> onMessage(StringBuilder message, Update update) throws BetMessageException {
        ...
        Matcher matcher = PlayTypeContent.PATTERN_LONG.matcher(message.toString());
        while (matcher.find()) {
            ...
            for (DicePrizeEnumsE prizeEnumsE : DicePrizeEnumsE.values()) {
                if (DiceBetUtil.strEquals(prizeEnumsE.getValue(), play)) {
                    BetEntity betEntity = new BetEntity(null, prizeEnumsE.getCode(), PlayTypeContent.PLAY_TYPE_LONG, amount);
    ...
    public boolean support(String message) {
        ...
        return PlayTypeContent.PATTERN_LONG.matcher(message).find();
    }
