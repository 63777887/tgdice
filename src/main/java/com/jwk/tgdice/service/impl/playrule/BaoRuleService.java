 ...
 */
@Service
public class BaoRuleService implements RuleService {
    ...
    public List<BetEntity> onMessage(StringBuilder message, Update update) throws BetMessageException {
        ...
        Matcher matcher = PlayTypeContent.PATTERN_BAOZI.matcher(message.toString());
        while (matcher.find()) {
            ...
            for (DicePrizeEnumsE prizeEnumsE : DicePrizeEnumsE.values()) {
                if (DiceBetUtil.strEquals(prizeEnumsE.getValue(), play)) {
                    BetEntity betEntity = new BetEntity(null, prizeEnumsE.getCode(), PlayTypeContent.PLAY_TYPE_BAOZI, amount);
    ...
    public boolean support(String message) {
        ...
        return PlayTypeContent.PATTERN_BAOZI.matcher(message).find();
    }
